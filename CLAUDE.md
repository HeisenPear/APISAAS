...
- **Le déploiement preview tape aussi la base de PRODUCTION.** Un banc le tient
  désormais : `tests/unit/e2eNecritPasEnProduction.test.ts` refuse toute écriture
  HTTP dans `tests/e2e/`, sauf déclaration écrite avec préfixe `E2E — ` et
  nettoyage. Constat du jour où il a été écrit : les huit parcours sont en
  lecture seule, et rien ne le gardait. Un e2e qui écrit écrit chez de vrais
  clients.
...
`app/utils/prixLigne.ts` porte le **cœur de la formule de prix**, atteignable
des deux côtés — `server/utils/pricing.ts` le RÉEXPORTE au lieu de le
redéfinir. Il existe parce que « le bug d'origine » est revenu par l'écran :
`argentUneSeuleRegle` ne balayait que `server/`, et les douze expressions
`quantité × prixUnitaire` des PAGES étaient hors de sa portée. Le bon de
livraison imprimait 100 € pour 2 500 € de miel, à côté d'une base qui stockait
2 500 € et d'une facture qui les réclamait.
`tests/unit/app/argentDansLesPages.test.ts` ferme l'autre moitié.

> **DEUX LECTURES D'UNE LIGNE, ET ELLES NE SE CONFONDENT PAS.**
> `montantLigneHt` fait gagner le total STOCKÉ — juste pour un document AU
> DOSSIER, dont la facture reprendra ce total tel quel. Dans un FORMULAIRE,
> `total` est un champ mort : posé à 0 à la création de la ligne, jamais remis à
> jour pendant la frappe. Le lire, c'est afficher 0 € pendant que l'apiculteur
> tape ses montants — le défaut inverse, que j'ai failli introduire en
> corrigeant celui-ci. D'où `montantSaisiHt` / `sommeSaisieHt`, et une règle qui
> refuse qu'un `*Form.vue` appelle la version « au dossier ».
...
| `emetteur.ts`              | Les colonnes de `profils` qu'un document imprime. La sélection existait en QUATRE exemplaires, dont trois oubliaient le logo — et le bon de livraison n'en avait aucune : il sortait ANONYME                                                                                                                                                                                                                 |
| `bonLivraisonStock.ts`     | **Quand le stock d'un bon part, et de combien.** `STATUTS_QUI_TIENNENT_LE_STOCK` est le SEUL endroit où se décide « à la création ou à la livraison »                                                                                                                                                                                                                                                        |
...
| `dateDite.ts`              | « Demain », « mardi », « le 15 mars » — la date que l'apiculteur a DITE. Le jour de référence se lit à PARIS (`partiesParis`) : à 00 h 30, il est encore la veille en UTC, et « demain » donnerait aujourd'hui. La valeur rendue est à minuit UTC (`jourUtc`), parce qu'on la STOCKE. Un jour inexistant — « le 31 février » — est REFUSÉ, jamais reporté en silence                                         |
| `forumSignalement.ts`      | Les compteurs du forum, RECALCULÉS jamais incrémentés. Un signalement `retabli` sort du dénombrement mais reste au dossier : sans ça, l'arbitrage serait rendu et le message resterait masqué — invisible depuis l'écran d'administration, visible seulement depuis le forum                                                                                                                                 |
| `forumNotification.ts`     | Prévenir sans harceler : jamais pour son propre message, une par fil et par jour, et seulement si la catégorie n'est pas coupée. NE LÈVE JAMAIS — le message est publié avant, et faire échouer la route ferait réécrire une réponse déjà enregistrée                                                                                                                                                        |
...
> **PLANIFIER N'EST PAS INVENTER.** `RE_INTENTION_FUTURE` refuse toute écriture
> sur une phrase au futur — à raison pour « il faut acheter du candi », qui
> fabriquerait une dépense inexistante. Mais un PLAN qu'on demande à consigner
> (« programme une visite demain ») est l'inverse : le futur y est le contenu,
> et refuser d'écrire, c'est refuser le service. La distinction ne tient pas au
> temps du verbe mais à ce dont on parle — un verbe de planification PLUS le nom
> d'une visite (`estVisitePlanifiee`). Le banc teste les DEUX sens : ne vérifier
> que « la visite passe » rouvrirait le trou de la dépense fantôme au premier
> élargissement du motif.
>
> Corollaire trouvé en le fermant : la VENTE n'avait pas la garde que l'achat
> porte depuis longtemps. « Il faudrait vendre les pots de printemps »
> enregistrait une vente — et une recette inventée gonfle le bilan qu'on
> déclare, là où une charge inventée ne fait que l'alourdir.

> **SURVEILLER N'EST PAS NOTIFIER.** `/maya` annonce publiquement « 26 situations
> surveillées » et « 6 familles de règles », calculées depuis
> `alertesCategories.ts`. Y ajouter une notification sociale — « on a répondu à
> votre sujet » — aurait fait passer la page à 27 et 7 : une promesse
> commerciale gonflée d'une unité par accident. Le moteur ne guette pas une
> réponse de forum, aucune règle ne la produit. D'où deux lectures nommées de la
> même table : `CATEGORIE_PAR_TYPE` (quel interrupteur gouverne quelle alerte)
> et `SITUATIONS_SURVEILLEES` / `FAMILLES_SURVEILLEES` (ce que la page annonce).
...
| **L'abonnement du voisin tient lieu du sien**    | Le balayage de répercussion suit les composables pour l'abonnement. `/stocks/alertes` appelle `useStocks()`, qui s'abonne pour SON compte : la page héritait de « quelqu'un écoute » alors que cette écoute-là ne rappellerait jamais son chargeur local. Retirer l'abonnement de la page restait VERT.                                                                                                                                                                                                                                                                                                    | Une règle qui regarde l'autre bout : un chargeur lancé au **montage** doit reparaître dans un rappel du bus          |
| **La sonde aveugle à une majuscule**             | La classe `[a-z0-9…]` d'une URL ne reconnaissait pas `` `/api/ruches/${rucheId.value}/cire` `` — un `I` capital. Affiner la règle ne faisait rien tomber : elle ne voyait plus rien à affiner.                                                                                                                                                                                                                                                                                                                                                                                                             | Exiger que la sonde **examine** un nombre non nul de cas, et le vérifier dans le banc                                |
| **Deux lectures d'une donnée, confondues**       | « À quel domaine cette route appartient-elle ? » (sa PREMIÈRE ressource) et « que rend cet appel-là ? » (sa DERNIÈRE) sont deux questions. Les confondre faisait prendre `/api/stocks/alertes` pour le domaine `alerte`, et réclamait qu'une fiche de ruche recharge sa CIRE sur un événement qui n'existe pas.                                                                                                                                                                                                                                                                                            | Nommer les deux questions, écrire deux fonctions, et **pin leurs bords** par des cas fabriqués                       |
...
| **Le réexport sans liaison**                     | `export { x } from '…'` n'ouvre un chemin que POUR LES AUTRES : dans le corps du module, `x` n'existe pas. `computeFactureTotals` levait `ReferenceError` — donc toute création et toute édition de facture. Et **`npm run typecheck` restait à ZÉRO erreur** (mesuré) : Nitro déclare les exports de `server/utils/` comme des globales d'auto-import, si bien que TypeScript trouvait le nom sans se demander si le module qui l'exporte peut se l'auto-importer à lui-même.                                                                                                                             | Importer ET réexporter ; `tests/unit/server/reexportSansLiaison.test.ts` le tient pour tout le serveur               |
| **La sonde satisfaite par la PROSE**             | Trois sondes d'un coup. La règle « toute route privée s'authentifie » cherchait `requireAuth` dans le fichier ENTIER : une route PUBLIQUE dont l'en-tête explique « ⚠️ PAS DE `requireAuth` » passait pour gardée — le mot suffisait, et il disait le contraire. `schemaEtSql` cherchait chaque colonne dans le SQL entier : « lue » se trouvait dans « Table interne lue uniquement côté serveur », donc une colonne réellement oubliée serait passée et toute requête sur sa table aurait échoué au déploiement. Et le garde d'écriture e2e, écrit le même jour, est tombé dedans à sa première version. | Blanchir AVANT de chercher — `sansCommentaires` pour le code, un équivalent qui suit les chaînes pour le SQL (`--`)  |
| **La règle refaite dans son propre cas**         | Le garde e2e blanchissait les commentaires dans le balayage, et son cas « fabriqué » refaisait le blanchiment de son côté : retirer `sansCommentaires` du balayage laissait TOUT VERT. Le cas ne mesurait que sa propre recopie de la règle.                                                                                                                                                                                                                                                                                                                                                               | Une seule fonction appelable (`ecritEnBase`), utilisée par le balayage ET par les cas fabriqués                      |
| **Le préfixe pris pour le mot entier**           | « Coupe sur un mot entier » se vérifiait par « le slug, tirets en espaces, est contenu dans le titre ». Or une coupe EN PLEIN MOT reste un préfixe : « …de la rein » y figure aussi bien que « …de la ». Neutraliser la coupe laissait le cas vert — mesuré.                                                                                                                                                                                                                                                                                                                                               | Affirmer le plus étroit : chaque morceau du slug est un mot COMPLET du titre                                         |
| **Le chiffre juste qui fait rougir**             | Quatre bancs exigeaient « 6 » en toutes lettres (catégories de notification). Ajouter une septième les faisait tomber pour un CHIFFRE, alors que leur règle — « aucune catégorie n'est éteinte par défaut », « seules les clés de catégorie survivent » — restait vraie. Un banc qui tombe sur un chiffre juste apprend à être ignoré, et c'est ainsi qu'une règle finit désactivée à côté du chiffre qu'on corrige.                                                                                                                                                                                       | Le compte se DÉRIVE de la source de vérité, dans les bancs comme dans les pages                                      |

> **UNE RÈGLE ENFERMÉE DANS SON `it` NE SE VÉRIFIE QUE SUR UN DÉPÔT SALE.**
> Quand le dépôt est propre, neutraliser la sonde donne exactement le même vert
> que la respecter — aucune mutation ne la tue. La parade est celle de
> `scripts/controle-sonde.mjs`, transposée aux bancs : extraire la règle en
> **fonction appelable**, puis lui présenter deux sources **fabriquées** — une
> qui la viole, une qui la respecte — et exiger qu'elle les distingue. Sans ce
> contrôle positif, rendre la sonde permissive (regarder tout le fichier au lieu
> des seuls rappels du bus) ne fait rien tomber.
...
- **Un correcteur d'orthographe ne sert QUE les chemins qui l'appellent.**
  `classifierTour` corrige avant de router — mais `resoudreFluxIntervention`
  RELIT les messages bruts pour retrouver son pivot et n'appliquait que
  `normaliser`. Une lettre de trop suffisait donc à faire échouer une phrase que
  `corrigerTexte` savait réparer, et c'était le chemin qui en avait le plus
  besoin : le geste dicté d'une main, au rucher, est celui qui porte des fautes
  de frappe. Corrigé, la robustesse mesurée passe de 75,9 % à 82,4 % (lettre en
  trop) et de 72,2 % à 77,8 % (lettres inversées).
...
PLAYWRIGHT_CHROMIUM_PATH=… npm run controle:sonde   # la sonde se teste elle-même
npm run build:e2e
