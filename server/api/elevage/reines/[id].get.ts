...
export default defineEventHandler(async (event) => {
  ...
  const [[reineRow], tests] = await Promise.all([
    db.select({ reine: reinesElevage, ligneeNom: lignees.nom, ligneeRace: lignees.race })
      .from(reinesElevage)
      .leftJoin(lignees, eq(reinesElevage.ligneeId, lignees.id))
      .where(and(eq(reinesElevage.id, id!), eq(reinesElevage.userId, user.id))).limit(1),
    db.select().from(testsPerformance)
