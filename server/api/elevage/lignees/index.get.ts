...
export default defineEventHandler(async (event) => {
  ...
  const [rows, [countResult]] = await Promise.all([
    db.select().from(lignees).where(where).orderBy(desc(lignees.createdAt)).limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)::int` }).from(lignees).where(where),
  ]);
