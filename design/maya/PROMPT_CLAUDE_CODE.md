...
1. **Copie le dossier `handoff/` dans ton repo** (branche `v2`), par exemple à la racine sous `design/maya/`.
   ...
   *(Optionnel mais utile : copie aussi le dossier `maya/` complet — la maquette `index.html` + les `.jsx` — pour que Claude Code puisse lire les valeurs exactes. Mets-le sous `design/maya/mockup/`.)*
...
| **P1** | Maya pense (`think`) pendant une réponse, ⌘K a "Demander à Maya", bouton central de la tab bar = Maya | Champ de texte libre géant mis en avant |
