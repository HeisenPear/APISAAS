<!--
  ...
  ⚠️ Les keyframes et classes `.maya-state-*` sont GLOBALES : colle le bloc de
  `maya.css` dans `app/assets/css/main.css`. Ce composant est purement présentiel.

  Emplacement cible : app/components/ia/MayaMark.vue  → s'utilise <IaMayaMark … />

  NB intégration APIGO : la version déposée dans le repo utilise `useId()` (SSR-safe)
  au lieu de `Math.random()` pour les ids de <defs>, afin d'éviter un mismatch
  d'hydratation Nuxt. Cette copie reste la référence design d'origine.
  ...
  Props :
    ...
    state  'static' | 'idle' | 'think' | 'listen' | 'alert' | 'loading' | 'success'
           static  : figé (listes denses, tableaux)
           idle    : respiration + scintillement — présence au repos
           think   : vague qui parcourt les alvéoles — Maya déroule ses règles (déterministe)
           listen  : onde concentrique — capture vocale en cours
           alert   : une seule alvéole s'embrase — une chose précise à regarder
           loading : une lueur fait le tour de la couronne — patience
           success : le miel monte du bas — récolte / réussite
...
<template>
  ...
  >
    ...
    <svg :width="size" :height="size" viewBox="0 0 24 24" class="maya-svg">
      ...
      <!-- 7 alvéoles (nth-child 2 = centre, 3..8 = couronne) — ciblées par maya.css -->
...
// Ids uniques par instance (évite la collision des <defs> quand plusieurs marks coexistent)
