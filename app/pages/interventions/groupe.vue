<template>
  <div class="space-y-6">
    <!-- Back link -->
    <NuxtLink to="/interventions" class="inline-flex items-center gap-1.5 text-xs font-medium transition-colors" style="color:var(--text-tertiary)">
    ...
    <div class="flex items-center gap-4">
      <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-[12px]" style="background:var(--sage-soft)">
        <UIcon name="i-lucide-layers" class="h-5 w-5" style="color:var(--sage-deep)" />
      </div>
      <div>
        <h1 class="text-[26px] font-semibold tracking-[-0.02em]" style="color:var(--text-primary)">Intervention groupée</h1>
        <p class="text-sm" style="color:var(--text-secondary)">Appliquez la même action sur plusieurs ruches simultanément</p>
      </div>
    </div>
    ...
    <div class="flex items-center gap-2">
      <div v-for="(label, i) in stepLabels" :key="i" class="flex items-center gap-2">
        <div
          ...
          :class="step > i + 1 ? 'bg-[var(--sage)] text-white' : step === i + 1 ? 'bg-[var(--honey)] text-white' : 'bg-[var(--surface-muted)] text-[var(--text-tertiary)]'"
        >
          <UIcon v-if="step > i + 1" name="i-lucide-check" class="h-3 w-3" />
          <span v-else>{{ i + 1 }}</span>
        </div>
        <span class="text-xs font-medium transition-colors" :class="step === i + 1 ? 'text-[var(--honey-deep)]' : 'text-[var(--text-tertiary)]'">{{ label }}</span>
        <div v-if="i < stepLabels.length - 1" class="h-px w-6" style="background:var(--border-default)" />
      </div>
    </div>
    ...
    <div v-if="step === 1" class="space-y-4">
      <div class="rounded-[14px] border bg-white p-6 shadow-sm" style="border-color:var(--border-default)">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
          <p class="text-[11px] font-semibold uppercase tracking-[0.12em]" style="color:var(--honey-deep)">Sélectionner les ruches</p>
          <div class="flex items-center gap-2">
            <select
            ...
            <button
              ...
              style="border-color:var(--border-default);color:var(--text-secondary);background:var(--surface-muted)"
        ...
        <div v-else class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          ...
          >
            ...
            <div
              ...
              :style="isSelected(ruche.id) ? 'border-color:var(--honey);background:var(--honey)' : 'border-color:var(--border-default);background:white'"
            >
              <UIcon v-if="isSelected(ruche.id)" name="i-lucide-check" class="h-2.5 w-2.5 text-white" />
            </div>

            <UIcon name="i-lucide-box" class="h-6 w-6" :style="isSelected(ruche.id) ? 'color:var(--honey)' : 'color:var(--text-tertiary)'" />
            <span class="text-sm font-semibold" style="color:var(--text-primary)">{{ ruche.numero }}</span>
            <span class="text-[11px]" style="color:var(--text-tertiary)">{{ ruche.type }}</span>
            <span v-if="ruche.rucherNom" class="truncate text-[10px]" style="color:var(--text-tertiary)">{{ ruche.rucherNom }}</span>
          </button>
        </div>
      </div>
      ...
      >
        <span class="text-sm font-medium" style="color:var(--honey-deep)">
          ...
          ruche{{ selectedRucheIds.length > 1 ? 's' : '' }} sélectionnée{{ selectedRucheIds.length > 1 ? 's' : '' }}
        </span>
        <UButton label="Continuer" icon="i-lucide-arrow-right" color="primary" size="sm" @click="step = 2" />
      </div>
    </div>
    ...
    <div v-if="step === 2" class="space-y-4">
      <div class="rounded-[14px] border bg-white p-6 shadow-sm" style="border-color:var(--border-default)">
        <div class="mb-4 flex items-center gap-2 text-sm">
          <div class="flex items-center gap-1.5 rounded-[8px] px-2.5 py-1" style="background:var(--honey-soft)">
            <UIcon name="i-lucide-layers" class="h-3.5 w-3.5" style="color:var(--honey-deep)" />
            <span class="font-medium" style="color:var(--honey-deep)">{{ selectedRucheIds.length }} ruche{{ selectedRucheIds.length > 1 ? 's' : '' }}</span>
          </div>
          <button type="button" class="text-xs font-medium hover:underline" style="color:var(--honey-deep)" @click="step = 1">
            Modifier
          </button>
        </div>
    ...
    <div v-if="step === 3" class="space-y-6">
      ...
      <div class="flex flex-wrap items-center gap-2 text-sm">
        ...
        <button type="button" class="text-xs font-medium hover:underline" style="color:var(--honey-deep)" @click="step = 2">
          Modifier
        </button>
      </div>

      <!-- Impact banner -->
      <div class="flex items-center gap-3 rounded-[12px] border px-4 py-3" style="border-color:color-mix(in srgb,var(--honey) 30%,transparent);background:var(--honey-soft)">
        <UIcon name="i-lucide-info" class="h-4 w-4 shrink-0" style="color:var(--honey-deep)" />
        <p class="text-sm" style="color:var(--text-secondary)">
          Ces paramètres seront appliqués à <strong style="color:var(--text-primary)">{{ selectedRucheIds.length }} ruche{{ selectedRucheIds.length > 1 ? 's' : '' }}</strong> simultanément.
          Les catégories <strong>Déplacement</strong>, <strong>Empilement</strong> et <strong>Transvasement</strong> ne sont pas disponibles en mode groupé.
        </p>
      </div>
      ...
      <div class="flex items-center justify-end gap-3">
        ...
        <UButton icon="i-lucide-check" color="primary" :loading="saving" @click="handleSubmit">
          Enregistrer sur {{ selectedRucheIds.length }} ruche{{ selectedRucheIds.length > 1 ? 's' : '' }}
        </UButton>
      </div>
    </div>
  </div>
...
const { data: ruchesData, status: ruchesStatus } = useFetch<ApiListResponse<Ruche & { rucherNom?: string }>>(
  '/api/ruches',
  { key: 'groupe-ruches-list', query: { limit: 200 }, default: () => ({ data: [], pagination: { page: 1, limit: 200, total: 0, totalPages: 0 } }) },
);
...
const filteredRuches = computed(() => filterRucherId.value ? allRuches.value.filter((r) => r.rucherId === filterRucherId.value) : allRuches.value);

function isSelected(id: string) { return selectedRucheIds.value.includes(id); }
