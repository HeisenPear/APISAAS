    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="mt-1 text-[13.5px] text-[var(--text-secondary)]">
          Registre des visites du vétérinaire sanitaire (Art. 3, arrêté du 5 juin 2000)
        </p>
      </div>
      <UButton
        icon="i-lucide-plus"
      ...
      <div class="mb-8">
        ...
        <div v-else class="space-y-3">
          ...
          >
            <!-- Date badge -->
            <div
              class="shrink-0 w-12 text-center bg-[var(--surface-muted)] rounded-[10px] py-2 px-1"
            >
              <p class="text-[11px] font-semibold text-[var(--text-tertiary)] uppercase">
                {{ new Date(visite.dateVisite).toLocaleDateString('fr-FR', { month: 'short' }) }}
              </p>
              <p class="text-[22px] font-semibold leading-none text-[var(--text-primary)]">
                {{ new Date(visite.dateVisite).getDate() }}
              </p>
            </div>
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p
                    v-if="visite.veterinaireNom"
                    class="text-[13px] text-[var(--text-secondary)] mt-0.5"
                  >
                    Dr. {{ visite.veterinaireNom }}
                  </p>
                  <p v-if="visite.rucherNom" class="text-[13px] text-[var(--text-secondary)]">
                    Rucher : {{ visite.rucherNom }}
                  </p>
                </div>
                <span
              ...
              <p
                v-if="visite.observations"
                class="text-[13px] text-[var(--text-tertiary)] mt-2 line-clamp-2"
              >
                {{ visite.observations }}
              </p>
            </div>
        ...
        >
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              >
                <td class="px-4 py-2.5 text-[var(--text-primary)] font-medium">
                  {{ new Date(visite.dateVisite).toLocaleDateString('fr-FR') }}
                </td>
        ...
        <div class="p-6 space-y-4">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-sm font-medium text-[var(--text-secondary)] mb-1"
                >Date *</label
              >
              <UiMobileDatePicker v-model="form.dateVisite" mode="date" />
            </div>
            <div>
              <label class="block text-sm font-medium text-[var(--text-secondary)] mb-1"
                >Rucher</label
          ...
          <div>
            ...
            <textarea
              ...
              rows="3"
              class="w-full rounded-xl border border-[var(--border-default)] bg-white px-3 py-2 text-sm focus:border-[var(--honey)] focus:outline-none resize-none"
              placeholder="Observations du vétérinaire..."
            />
          </div>
          <div>
            <label class="block text-sm font-medium text-[var(--text-secondary)] mb-1"
