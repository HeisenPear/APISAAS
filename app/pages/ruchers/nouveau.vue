<template>
  <div class="space-y-6">
    <!-- Back link -->
    <NuxtLink to="/ruchers" class="inline-flex items-center gap-1.5 text-xs font-medium transition-colors" style="color:var(--text-tertiary)" @mouseenter="($event.currentTarget as HTMLAnchorElement).style.color = 'var(--text-secondary)'" @mouseleave="($event.currentTarget as HTMLAnchorElement).style.color = 'var(--text-tertiary)'">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-[26px] font-semibold tracking-[-0.02em]" style="color:var(--text-primary)">Nouveau rucher</h1>
        <p class="text-sm" style="color:var(--text-secondary)">Ajoutez un nouvel emplacement pour vos ruches</p>
      </div>
    </div>
