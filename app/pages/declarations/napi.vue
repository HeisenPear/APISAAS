...
interface DeclarationRow {
  ...
  annee: number;
  statut: string;
  nombreTotalColonies: number;
  ruchersData: unknown[] | null;
  dateDeclaration: string;
}
interface PrefillData {
  cheptel: { total: number; nbProduction: number; nbRuchettes: number; nbNuclei: number };
  ruchers: unknown[];
}
...
const alreadyDeclared = computed(
  () => currentDecl.value && currentDecl.value.statut !== 'brouillon',
);
...
<template>
  <div>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="mt-1 text-[13.5px] text-[var(--text-secondary)]">
          Déclaration annuelle de détention et d'emplacement de ruches (Cerfa 13995*07)
        </p>
      </div>
      <UButton
    ...
    <!-- Info banner -->
    <div
      class="bg-[var(--honey-soft)] border border-[var(--honey)] rounded-[12px] p-4 mb-6 flex items-start gap-3"
    >
      <UIcon
        name="i-lucide-calendar-clock"
        class="h-5 w-5 text-[var(--honey-deep)] shrink-0 mt-0.5"
      />
      <div>
        <p class="text-[13px] font-semibold text-[var(--honey-deep)]">
          Période de déclaration : 1er septembre — 31 décembre
        </p>
        <p class="text-[12.5px] text-[var(--text-secondary)] mt-0.5">
          La déclaration NAPI est obligatoire chaque année. Tout apiculteur doit déclarer son
          cheptel avant le 31 décembre.
        </p>
      </div>
    </div>
    ...
    <div class="mb-8">
      <div class="flex items-center justify-between mb-3">
        <p class="text-[11px] font-semibold text-[var(--honey-deep)] uppercase tracking-[0.12em]">
          01 — Déclarations
        </p>
        <div class="flex items-center gap-2">
          <span class="text-[12px] text-[var(--text-tertiary)]">Année</span>
          <select
            class="text-[12px] border border-[var(--border-default)] rounded-lg px-2 py-1 bg-white text-[var(--text-primary)] focus:border-[var(--honey)] focus:outline-none"
            :value="currentYear"
          >
            <option
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-4 py-3 text-[var(--text-secondary)]">
                {{ decl.ruchersData?.length ?? '—' }}
              </td>
              <td class="px-4 py-3 text-[var(--text-secondary)]">
                {{ new Date(decl.dateDeclaration).toLocaleDateString('fr-FR') }}
              </td>
              <td class="px-4 py-3">
                <span
                  class="rounded-full px-2.5 py-1 text-[11px] font-semibold"
                  :style="
                    decl.statut === 'recepisse_recu'
                      ? 'background:var(--sage-soft);color:var(--sage-deep)'
                      : decl.statut === 'enregistre'
    ...
    <div>
      ...
      <div class="space-y-3">
          ...
          v-for="(step, i) in [
            {
              ...
              desc: 'Nom, adresse et numéro NAPI à jour dans les paramètres',
              to: '/parametres',
            },
            {
              icon: 'i-lucide-map-pin',
              title: 'Renseigner les communes',
              desc: 'Chaque rucher doit avoir une commune associée pour le formulaire Cerfa',
            },
            {
              icon: 'i-lucide-grid-2x2',
              title: 'Compter vos colonies',
              desc: 'Ruches de production, ruchettes et nuclei sont à déclarer séparément',
            },
            {
              icon: 'i-lucide-download',
              title: 'Télécharger le Cerfa pré-rempli',
              desc: 'Cerfa 13995*07 généré automatiquement avec vos données',
              action: true,
            },
          ]"
          :key="i"
        ...
        >
          <div
            class="w-8 h-8 rounded-[8px] bg-[var(--surface-muted)] flex items-center justify-center shrink-0"
          >
          ...
          <NuxtLink
            v-if="step.to"
            :to="step.to"
            class="text-[12px] font-semibold text-[var(--honey-deep)] hover:underline shrink-0"
            >Configurer</NuxtLink
          >
          <button
            v-if="step.action"
    ...
    <template v-if="!inPeriod && !isPrepPeriod">
      <div
        class="mt-6 rounded-[14px] border border-[var(--border-default)] bg-white p-8 text-center"
      >
        <UIcon name="i-lucide-calendar" class="mx-auto h-10 w-10 text-[var(--text-tertiary)]" />
        <h2 class="mt-4 text-[17px] font-semibold text-[var(--text-primary)]">
          Hors période de déclaration
        </h2>
        <p class="mt-2 text-[13.5px] text-[var(--text-secondary)]">
          La période s'ouvre le <strong>1er septembre</strong> et se termine le
          <strong>31 décembre</strong>.
        </p>
      </div>
    </template>
    ...
    <template v-if="alreadyDeclared && currentDecl">
      <div
        class="mt-6 rounded-[14px] border border-[var(--sage-soft)] bg-[var(--sage-soft)] p-5 flex items-center gap-3"
      >
        ...
        <div>
          <p class="text-[13px] font-semibold text-[var(--sage-deep)]">
            ...
            <span class="font-normal text-[var(--text-secondary)] ml-1"
              >le {{ new Date(currentDecl.dateDeclaration).toLocaleDateString('fr-FR') }}</span
            >
          </p>
          <p class="text-[12.5px] text-[var(--text-secondary)]">
            {{ currentDecl.nombreTotalColonies }} colonies déclarées
          </p>
        </div>
      </div>
    </template>
      ...
      <template #content>
        <div class="p-6 space-y-4">
          ...
          <p class="text-[13.5px] text-[var(--text-secondary)]">
            Confirmez-vous avoir effectué votre déclaration officielle sur le téléservice DGAL pour
            l'année {{ currentYear }} ?
          </p>
          <UInput
            v-model="formNotes"
