...
<template>
  <div class="space-y-8">
    ...
    <section class="space-y-3" data-tutorial="elevage-lignees">
      ...
      <!-- Loading -->
      <div v-if="reinesPending" class="bg-white border border-[var(--border-default)] rounded-[12px] overflow-hidden">
      ...
      >
        ...
        <UButton to="/elevage/reines" size="sm" color="primary" variant="soft">Ajouter une reine</UButton>
      </div>
      ...
      <div v-else class="bg-white border border-[var(--border-default)] rounded-[12px] overflow-hidden">
        <table class="w-full">
          <thead class="bg-[var(--surface-muted)]">
            <tr>
              <th class="px-5 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-[0.08em] text-[var(--text-tertiary)]">Identifiant</th>
              <th class="hidden px-5 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-[0.08em] text-[var(--text-tertiary)] sm:table-cell">Lignée</th>
              <th class="hidden px-5 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-[0.08em] text-[var(--text-tertiary)] md:table-cell">Ruche</th>
              <th class="hidden px-5 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-[0.08em] text-[var(--text-tertiary)] md:table-cell">Origine</th>
              <th class="hidden px-5 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-[0.08em] text-[var(--text-tertiary)] sm:table-cell">Date intro</th>
              <th class="px-5 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-[0.08em] text-[var(--text-tertiary)]">Qualité</th>
            </tr>
          </thead>
            ...
            >
              <td class="px-5 py-3">
                <div class="flex items-center gap-2">
                  ...
                  />
                  <UIcon v-else name="i-lucide-crown" class="h-3.5 w-3.5 shrink-0 text-[var(--honey)]" />
    ...
    <section class="space-y-3" data-tutorial="elevage-sessions">
      ...
      <div v-if="sessionsPending" class="space-y-3">
        <div v-for="i in 3" :key="i" class="h-16 animate-pulse rounded-[14px] bg-[var(--surface-muted)]" />
      </div>
      ...
      >
        ...
        <UButton to="/elevage/greffage" size="sm" color="primary" variant="soft">Créer une session</UButton>
      </div>
      ...
      <div v-else class="space-y-2">
        ...
        >
          <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-[var(--honey-soft)]">
            <UIcon name="i-lucide-scissors" class="h-4 w-4 text-[var(--honey-deep)]" />
          </div>
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-[var(--text-primary)]">{{ formatDate(sess.dateGreffage) }}</p>
            <p class="text-xs text-[var(--text-secondary)]">
              {{ sess.nombreCellulesGreffees }} greffées
              <span v-if="sess.nombreCellulesAcceptees"> · {{ sess.nombreCellulesAcceptees }} acceptées</span>
