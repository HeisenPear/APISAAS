...
│   ├── MayaBubble.vue           → app/components/ia/MayaBubble.vue (bulle compagnon qui se déplie)
│   ├── MayaPresenceSettings.vue → app/components/ia/MayaPresenceSettings.vue (réglages présence)
...
| **Bulle compagnon** *(mode Discrète)* | **nouveau** `ia/MayaBubble.vue` *(fourni)* | Bouton noir bas-droite qui **se déplie en fenêtre** de conversation (morph). Réponses riches + actions au tap. Monté dans `layouts/default.vue`, affiché si présence = `discrete` | `MayaBubbleProto.jsx` (proto/) |
| **Réglages de présence** | **nouveau** `ia/MayaPresenceSettings.vue` *(fourni)* | Modale : choix Partout / Discrète / En pause + ce que Maya surveille (alertes, briefing, dictée) | `MayaSettings` (proto/MayaBubbleProto.jsx) |
| **Entrée sidebar** | `ui/AppSidebar*.vue` *(existe)* | Item « Maya · Assistant » (garder le mot « Assistant ») qui ouvre les réglages de présence | `sidebar2.jsx` |
...
### 7bis. Mode de présence (Maya partout vs bulle discrète)

Maya a **deux personnalités** d'une même identité, choisies par l'utilisateur :
- **Partout** (proactive) : cartes d'aide, briefing, alertes — tout le §7 ci-dessus.
- **Discrète** (sur demande) : **aucune** carte ni notification de fond, juste une petite bulle chatbot en bas à droite qu'on ouvre à la demande.
- **En pause** : Maya en retrait total.

À créer / fourni :
- **État global** : `app/stores/maya.ts` (Pinia) ou `composables/useMayaPresence.ts` — `presence: 'partout' | 'discrete' | 'pause'`, persisté (localStorage + préférence compte). Les surfaces proactives (`MayaCard`, briefings, cartes contextuelles) ne s'affichent QUE si `presence === 'partout'`.
- **`ia/MayaBubble.vue`** *(FOURNI dans `components/`)* : le bouton qui **se déplie en fenêtre** (morph), popover chatbot ~392×580, réponses riches + actions au tap, états du logo câblés. Monté globalement dans `app/layouts/default.vue`, affiché si `presence === 'discrete'`. Réf. interactive complète (ouverture, boop des alvéoles, flux de proposition, ton) : **`proto/MayaBubbleProto.jsx`** — c'est la source de vérité du comportement.
- **`ia/MayaPresenceSettings.vue`** *(FOURNI dans `components/`)* : la modale de réglages (Partout / Discrète / En pause + ce que Maya surveille). Ouverte depuis l'entrée sidebar « Maya · Assistant » ET depuis l'icône réglages de la bulle. Réf. : `MayaSettings` dans `proto/MayaBubbleProto.jsx`.
- **Planches statiques** (états du FAB, versions web/mobile, écran de présence) : `pages/bubble.jsx` (`MayaBubbleWeb`, `MayaBubbleStates`, `MayaBubbleMobile`, `MayaPresence`).
- **Mobile** : la bulle s'ouvre en **feuille du bas**. Le FAB cohabite avec `BottomNav` (le bouton central Maya ouvre la même bulle — ne pas dupliquer).

> Important : en mode discret, Maya **ne parle jamais sans qu'on l'appelle**. Les réponses restent déterministes (chips/voix). Le logo vivant et l'identité sont identiques — seule la présence change.
...
**P1 — Surfaces Maya (1–2 j).** Brancher les états (`think`/`listen`/`idle`) sur `useCopilote`. Restyler `MayaCard` (briefing). ⌘K : section "Demander à Maya". Créer `MayaPanel.vue` + `MayaLauncher.vue`. BottomNav : bouton central Maya. **Mode de présence** : `stores/maya.ts` + réglage Paramètres + `ui/MayaBubble.vue` (mode discret) montée dans `layouts/default.vue` (voir §7bis).
